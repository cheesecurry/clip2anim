...
from PySide6.QtCore import Qt, QUrl, QTime, QThread, Signal, QSize, QEvent
...
from custom_widget import TimelineSlider, VideoDisplayWidget
    ...
    def run(self):
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                logging.error(f"FFmpeg Error (Exit Code: {process.returncode})\n\n{error_msg}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.privacy_crop.setToolTip("画面下部を除去します") 
    ...
    def load_config(self):
        ...
        if config_path.exists():
            try:
                ...
                logging.info("設定ファイルの読み込みに失敗しました。")
                return {}
        logging.info("設定ファイルが見つかりませんでした。")
        return {}
    ...
    def _load_video(self, path):
        ...
        logging.info(f"動画ファイル「{path}」を読み込みました。")
    ...
    def on_export_finished(self, out_file: Path):
        ...
        logging.info(f"完了: {out_file.name}...\n({size_bytes/1024:.1f} KB)")
...
if __name__ == "__main__":
    # --- コマンドライン引数の設定 ---
    parser = argparse.ArgumentParser(description="Clip2Anim: 動画からアニメーションを作成するツール")
    
    # 動画ファイルは任意（指定があれば読み込む）
    parser.add_argument("video_file", nargs="?", help="対象の動画ファイルパス")
    
    # ログレベルの指定 (デフォルトは INFO)
    parser.add_argument(
        "--log-level", "-l",
        default="INFO",
        choices=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"],
        help="ログの出力レベルを指定します (default: INFO)"
    )
    ...
    if os.path.isfile(FFMPEG):
        logging.info(f"FFmpegインストールパス:{FFMPEG}")
    elif ffmpeg:
        # 環境変数に登録されているのでそれを採用する
        FFMPEG = Path(ffmpeg)
        logging.info(f"FFmpegインストールパス:{FFMPEG}")
    else:
        logging.error(f"FFmpegが見つかりません")

    app = QApplication(sys.argv)
    w = MainWindow()
    
    # 引数で動画ファイルが指定されている場合、ロードして表示
    if args.video_file and os.path.isfile(args.video_file):
        w._load_video(args.video_file)
